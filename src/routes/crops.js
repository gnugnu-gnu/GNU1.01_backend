...
router.get('/', (req, res) => {
  ...
  `, (err, rows) => {
    ...
    res.json(rows); // [{ id: 1, crop_name: '토마토', current_quantity: 50 }, ...]
  });
});
...
router.post('/', (req, res) => {
  ...
  db.run(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `, [bed_id, crop_type_id, current_quantity, initial_quantity, planted_date], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    res.status(201).json({ message: '작물 추가 완료', id: this.lastID });
  });
});
...
router.post('/:id/history', (req, res) => {
  ...
  db.run(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `, [cropId, type, date, details ? JSON.stringify(details) : null, quantity_change || 0], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    if (quantity_change) {
      db.run(`
      ...
      `, [quantity_change, cropId], (err) => {
        if (err) return res.status(500).json({ error: err.message });
        res.status(201).json({ message: '관리 기록 추가 완료', id: this.lastID });
      });
    } else {
      res.status(201).json({ message: '관리 기록 추가 완료', id: this.lastID });
    }
