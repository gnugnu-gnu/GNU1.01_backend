...
router.get('/', async (req, res) => {
  try {
    ...
    const result = crops.map(crop => ({
      id: crop._id,
      crop_name: crop.crop_type_id?.name || '알 수 없는 작물',
      current_quantity: crop.current_quantity ?? 0
    }));
    res.json(result);
  } catch (err) {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    // planted_date가 유효한지 확인
    const plantedDate = crop.planted_date instanceof Date && !isNaN(crop.planted_date)
      ? crop.planted_date
      : new Date();

    const lastWater = history.find(h => h.type === 'water')?.date || plantedDate;
    const lastFertilizer = history.find(h => h.type === 'fertilizer')?.date || plantedDate;
    const lastPesticide = history.find(h => h.type === 'pesticide')?.date || plantedDate;

    // cycles와 crop_type_id가 없으면 기본값 제공
    const cycles = {
      water: crop.cycles?.water || crop.crop_type_id?.water_cycle || 7,
      fertilizer: crop.cycles?.fertilizer || crop.crop_type_id?.fertilizer_cycle || 14,
      pesticide: crop.cycles?.pesticide || crop.crop_type_id?.pesticide_cycle || 30
    };

    const nextWater = getNextDate(
      lastWater instanceof Date && !isNaN(lastWater) ? lastWater.toISOString().split('T')[0] : plantedDate.toISOString().split('T')[0],
      cycles.water
    );
    const nextFertilizer = getNextDate(
      lastFertilizer instanceof Date && !isNaN(lastFertilizer) ? lastFertilizer.toISOString().split('T')[0] : plantedDate.toISOString().split('T')[0],
      cycles.fertilizer
    );
    const nextPesticide = getNextDate(
      lastPesticide instanceof Date && !isNaN(lastPesticide) ? lastPesticide.toISOString().split('T')[0] : plantedDate.toISOString().split('T')[0],
      cycles.pesticide
    );

    const result = {
      name: crop.crop_type_id?.name || '알 수 없는 작물',
      current_quantity: crop.current_quantity ?? 0,
      initial_quantity: crop.initial_quantity ?? 0,
      ...
      history: history.map(h => ({
        id: h._id,
        type: h.type,
        date: h.date instanceof Date && !isNaN(h.date) ? h.date.toISOString().split('T')[0] : today,
        details: h.details || {},
        quantity_change: h.quantity_change || 0
      }))
    };
    res.json(result);
  } catch (err) {
...
router.post('/', async (req, res) => {
  try {
    ...
    const crop = new Crop({
      ...
      cycles: {
        water: cropType.water_cycle || 7,
        fertilizer: cropType.fertilizer_cycle || 14,
        pesticide: cropType.pesticide_cycle || 30
      }
    });
...
router.get('/:id/export', async (req, res) => {
  try {
    ...
    const data = history.map(row => ({
      날짜: row.date instanceof Date && !isNaN(row.date) ? row.date.toISOString().split('T')[0] : 'N/A',
      관리_종류: row.type,
      세부사항: typeof row.details === 'object' ? JSON.stringify(row.details) : row.details || '',
    ...
    const plantedDate = crop.planted_date instanceof Date && !isNaN(crop.planted_date)
      ? crop.planted_date.toISOString().split('T')[0].replace(/-/g, '')
      : 'unknown';
    const fileName = `${crop.bed_id?.name || 'unknown'}-${crop.crop_type_id?.name || 'unknown'}-${crop.initial_quantity || '0'}-${plantedDate}.xlsx`;
